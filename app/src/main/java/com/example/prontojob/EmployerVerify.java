...
public class EmployerVerify extends AppCompatActivity implements View.OnClickListener {
    ...
    Button eDate, btnVerify, btnComCamera, btnComGallery, btnIDCam, btnIDGallery;
    Spinner eIDType;
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.comCameraBtn:
                comAskCameraPermission();
                break;
            case R.id.eIDCameraBtn:
                eIDAskCameraPermission();
                break;
    ...
    private void comAskCameraPermission() {
    ...
    private void uploadImageToFirebase(String name, Uri contentUri) {
        StorageReference image = storageReference.child("Employer's Company ID").child(userID).child("images/" + name);
    ...
    private void UploadFile() {
        if (filepath != null) {
            ...
            progressDialog.show();
            StorageReference sref = storageReference.child("Employer's COE").child(userID).child("Resume" + formattedDate + ".pdf");
    ...
    //method for asking permission for the government ID
    private void eIDAskCameraPermission() {
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(this,new String[] {Manifest.permission.CAMERA}, CAMERA_PERM_CODE);
        }else {
            eIDDispatchTakePictureIntent();
        }
    }

    //method for uploading the images to firebase
    private void eIDUploadImageToFirebase(String name, Uri contentUri) {
        StorageReference image = storageReference.child("Employer's Government ID").child(userID).child("images/" + name);
        image.putFile(contentUri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            @Override
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                image.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
    ...
    //get the file extension
    private String getFileExtEID(Uri contentUri) {
        ContentResolver c = getContentResolver();
        MimeTypeMap mime = MimeTypeMap.getSingleton();
        return mime.getExtensionFromMimeType(c.getType(contentUri));
    }

    private File eIDCreateImageFile() throws IOException {
    ...
    private void eIDDispatchTakePictureIntent() {
        Intent takePictureIntent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
        // Ensure that there's a camera activity to handle the intent
        {
            // Create the File where the photo should go
